...
#line 169 "parser.y"
	{
        Factor i = create_factor_by_name(yystack.l_mark[-2].ident);
        factor_push(i);
        insert_code(Store);
...
#line 176 "parser.y"
	{
        Factor i = create_factor_by_name(yystack.l_mark[-5].ident);
        factor_push(i);
        insert_code(Load);
        set_cmp_type(SLE);
        insert_code(Icmp);
...
#line 185 "parser.y"
	{
        insert_code(BrUncond);
        insert_code(Label);
        Factor i = create_factor_by_name(yystack.l_mark[-8].ident);
        factor_push(i);
        insert_code(Load);
